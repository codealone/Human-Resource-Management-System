...
public class CheckContact extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					CheckContact frame = new CheckContact(id);
	...
	public CheckContact(String n1) {
		...
		btnYes.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					DisplayContact dContact = new DisplayContact(id);
					dContact.setVisible(true);
					setVisible(false);
				} catch (Exception et) {
					et.printStackTrace();
				}
